  ...
  if (searchArea == "Current Location") {
    ...
    var interval1 = setInterval(function() {
      if (lon != "undefined" && lat != "undefined") {
        ...
        queryUrl =
          "https://us-restaurant-menus.p.rapidapi.com/restaurants/search/geo?lon=" +
          lon +
          "&&lat=" +
          lat +
          "&distance=" +
...
function displayResult(htmlDivId, result) {
  ...
  if (result.length == 0) {
    ...
    for (i in result) {
      var row = $("<div>")
          .attr("class", "row container_div shadow rounded")
          .attr("id", i),
      ...
      var div1 = $("<h4>")
          .attr("class", "result")
          .text(result[i].restaurant_name),
        div2 = $("<div>")
          .attr("class", "result")
          .text("Address: " + result[i].address.formatted),
        div3 = $("<div>")
          .attr("class", "result")
          .text("Cuisines: " + result[i].cuisines.toString()),
        div4 = $("<div>")
          .attr("class", "result")
          .text("Phone: " + result[i].restaurant_phone),
        div5 = $("<div>")
          .attr("class", "result")
          .text("Price Range: " + result[i].price_range),
        div6 = $("<div>")
          .attr("class", "result")
          .text("Hours: " + result[i].hours);

      var img0 = $("<img>")
        .attr("src", "assets/images/reviews.png")
        .attr("class", "reviewsImg shadow mb-5 bg-white rounded");
      var btn0 = $("<a>")
      ...
      var img1 = $("<img>")
        .attr("src", "assets/images/GoogleMap.jpeg")
        .attr("class", "googleImg shadow mb-5 bg-white rounded");
      var btn1 = $("<a>")
      ...
      var img2 = $("<img>")
        .attr("src", "assets/images/share1.png")
        .attr("class", "shareImg shadow mb-5 bg-white rounded");
      var btn2 = $("<a>")
...
function displayReview(indexOfRestaurant) {
  ...
  var settings = {
    ...
    crossDomain: true,
    url:
      "https://tripadvisor1.p.rapidapi.com/restaurants/list-by-latlng?limit=50&currency=USD&distance=2&lunit=km&lang=en_US&latitude=" +
      selected_lat +
      "&longitude=" +
      selected_lon,
    method: "GET",
